...
def fix_cp1252codes(text):
    ...
    if re.search(u"[\x80-\x9f]", text):
        ...
        if isinstance(text, bytes):
            # декодируем байты в строку (Unicode)
            text = text.decode("iso-8859-1")
        # заменяем управляющие символы (используется юникод-строка)
        text = re.sub(r"[\x80-\x9f]", fixup, text)
    return text


def clean_utf8(text):
    return filter(lambda x: '\x1f' < x < '\x7f', text)
